...
function DocumentViewer({ item, onOpen }) {
  const StatusIcon = item.status === 'ready' ? CheckCircle2 : Clock
  const metadata = item.metadata || {}

  return (
    <button type="button" className="soft-card w-full rounded-[24px] p-4 text-left transition hover:border-mint-500 hover:shadow-lg hover:shadow-mint-500/10" onClick={() => onOpen?.(item)}>
