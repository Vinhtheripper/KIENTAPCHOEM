        ...
        <section className="surface-card rounded-[26px] p-5">
          ...
          <div className="space-y-4">
            ...
            {!loading && events.map((event) => (
              <article
                className="grid gap-3 rounded-[22px] border border-[#d8ede5] bg-white p-4 text-left md:grid-cols-[160px_1fr]"
                key={event.id}
              >
                <div>
                ...
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <span className="chip px-2 py-1 text-[10px]">Status: {event.status}</span>
                    {event.labels?.map((label) => <span className="chip px-2 py-1 text-[10px]" key={label}>{label}</span>)}
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2">
                    {event.content_id && <button className="btn-secondary min-h-10 px-3 py-2 text-sm" type="button" onClick={() => openContent(event.content_id)}>Open file</button>}
